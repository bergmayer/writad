...
final class LineManager {
    ...
    /// Mark a line as hidden (folded out of layout) or visible. Returns true
    /// when the state actually changed; the caller is then responsible for
    /// invalidating any cached layout that derives from the line's height.
    @discardableResult
    func setHidden(_ hidden: Bool, of line: DocumentLineNode) -> Bool {
        guard line.data.isHidden != hidden else { return false }
        line.data.isHidden = hidden
        documentLineTree.updateAfterChangingChildren(of: line)
        return true
    }
