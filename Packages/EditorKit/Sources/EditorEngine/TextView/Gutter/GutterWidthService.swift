...
final class GutterWidthService {
    ...
    /// Fixed pt reserved at the right edge of each line-number cell for the
    /// fold-indicator triangle. The line-number digits get the remaining
    /// width, so they never truncate when a triangle appears.
    var foldIndicatorReservation: CGFloat = 14 {
        didSet {
            if foldIndicatorReservation != oldValue {
                sendGutterWidthUpdatedIfNeeded()
            }
        }
    }
    var gutterWidth: CGFloat {
        if showLineNumbers {
            return lineNumberWidth + foldIndicatorReservation + gutterLeadingPadding + gutterTrailingPadding
        } else {
            return 0
        }
    }
